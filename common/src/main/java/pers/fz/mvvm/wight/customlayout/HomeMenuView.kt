...
open class HomeMenuView : ConstraintLayout, DefaultLifecycleObserver {
    ...
    fun initData(
        ...
        if (this.menuList.isNullOrEmpty() == true) {
            return
        }
        initImageRounds()
    ...
    protected val onPageChangeCallback: ViewPager2.OnPageChangeCallback =
        object : ViewPager2.OnPageChangeCallback() {
            override fun onPageSelected(position: Int) {
                ...
                dotsLayout.getChildAt(realPos).background = drawableResCurrent?:defaultDrawableResCurrent
                if (lastPos >= 0 && lastPos < dotsLayout.size && lastPos != realPos) {
                    dotsLayout.getChildAt(lastPos).background = drawableResNormal?:defaultDrawableResNormal
                }
                lastPos = realPos
            }
        }
