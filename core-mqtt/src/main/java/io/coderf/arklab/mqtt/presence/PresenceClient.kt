 ...
 * - 提供心跳 / 离线消息发布辅助方法
 * - 完整转发连接/断连/重连/错误/消息回调（不再只透传 onConnected）
 *
 * 应用层定时心跳请配合 [HeartbeatScheduler] 使用；
 * 完整生命周期编排见 [PresenceManager] 接口（由宿主 App 实现）。
 *
 * ## Java 接入示例
 * ```
...
class PresenceClient {
    ...
    fun connect(info: PresenceConfig, listener: PresenceListener) {
        if (info.brokerAddress.isBlank() || info.clientId.isBlank()
            ...
            MqttLog.logger(TAG, "MQTT 连接参数不完整，跳过连接")
            return
        }
        currentInfo = info
        ...
        connection.connect(config, object : AbstractMqttListener() {
            ...
            override fun onReconnecting(attempt: Int, maxAttempts: Int, nextRetryDelaySeconds: Int) {
                listener.onReconnecting(attempt, maxAttempts, nextRetryDelaySeconds)
            }
    ...
    fun connect(info: PresenceConfig, listener: MqttListener) {
        connect(info, object : AbstractPresenceListener() {
            ...
            override fun onReconnecting(attempt: Int, maxAttempts: Int, nextRetryDelaySeconds: Int) =
                listener.onReconnecting(attempt, maxAttempts, nextRetryDelaySeconds)
            override fun onReconnectExhausted() = listener.onReconnectExhausted()
            override fun onError(exception: MqttException?) = listener.onError(exception)
            override fun onMessage(topic: String, payload: String) = listener.onMessage(topic, payload)
            override fun onMessageRaw(message: MqttRawMessage) = listener.onMessageRaw(message)
            override fun onDeliveryComplete() = listener.onDeliveryComplete()
        })
    }
