...
class MqttConnection @JvmOverloads constructor(
    ...
    fun connect(config: MqttConfig, listener: MqttListener) {
        ...
        if (address.isNullOrBlank() || config.clientId.isBlank()) {
            MqttLog.logger(tag, "MQTT 连接参数不完整（address/clientId），跳过连接")
            return
        }
        if (config.requireAuth && (config.username.isBlank() || config.password.isBlank())) {
            MqttLog.logger(tag, "MQTT 鉴权参数不完整（username/password），跳过连接")
            return
        }
        synchronized(lock) {
            ...
            try {
                ...
                MqttLog.logger(tag, "MQTT 连接失败: ${e.message}")
                closeClientLocked()
    ...
    fun disconnect() {
        synchronized(lock) {
            ...
            try {
                ...
                MqttLog.logger(tag, "MQTT 断开异常: ${e.message}")
            } finally {
                closeClientLocked()
            }
        }
    ...
    fun publish(topic: String, message: MqttMessage): Boolean {
        synchronized(lock) {
            ...
            return try {
                ...
                MqttLog.logger(tag, "发送消息失败: ${e.message}")
                false
            }
        }
    }
    ...
    fun subscribe(topics: Array<String>, qos: IntArray? = null): Boolean {
        ...
        synchronized(lock) {
            ...
            return try {
                ...
                MqttLog.logger(tag, "MQTT 动态订阅失败: ${e.message}")
                false
            }
        }
    }
    ...
    fun unsubscribe(topics: Array<String>): Boolean {
        ...
        synchronized(lock) {
            ...
            return try {
                ...
                MqttLog.logger(tag, "MQTT 动态退订失败: ${e.message}")
                false
            }
        }
    }
    ...
    private fun subscribeIfNeeded(config: MqttConfig) {
        ...
        try {
            ...
            MqttLog.logger(tag, "MQTT 订阅失败: ${e.message}")
        }
    }
    ...
    private fun scheduleCustomReconnectLocked(config: MqttConfig) {
        ...
        if (reconnectAttempt > maxAttempts) {
            MqttLog.logger(tag, "MQTT 已达最大重连次数: $maxAttempts")
    ...
    private fun attemptCustomReconnectLocked(config: MqttConfig) {
        ...
        try {
            ...
            MqttLog.logger(tag, "MQTT 重连失败: ${e.message}")
            scheduleCustomReconnectLocked(config)
        }
    }
    ...
    override fun disconnected(disconnectResponse: MqttDisconnectResponse?) {
        MqttLog.logger(tag, "MQTT 已断开: $disconnectResponse")
    ...
    override fun mqttErrorOccurred(exception: MqttException?) {
        MqttLog.logger(tag, "MQTT 错误: ${exception?.message}")
        val config = synchronized(lock) { currentConfig } ?: return
        if (exception != null) {
    ...
    override fun connectComplete(reconnect: Boolean, serverURI: String?) {
        MqttLog.logger(tag, "MQTT 连接完成 reconnect=$reconnect uri=$serverURI")
        synchronized(lock) {
