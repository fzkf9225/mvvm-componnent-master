...
public class CsvFileLogger implements IFileLogger {
    ...
    public void startNewLog(String customFileName) {
        try {
            ...
            LogUtil.loggerI(GpsService.TAG, "Created new log file: " + currentFilePath);
        } catch (IOException e) {
            LogUtil.loggerI(GpsService.TAG, "Failed to start log: " + e.getMessage());
            isWriterReady = false;
        }
    }
    ...
    public void write(Location loc) {
        if (writer == null || !isWriterReady) {
            LogUtil.loggerI(GpsService.TAG, "Writer not initialized, cannot write location");
            return;
        }
        ...
        LogUtil.loggerI(GpsService.TAG, "Written to CSV: " + timeStr + "," + loc.getLatitude() + "," + loc.getLongitude());
    }

    @Override
    public void annotate(String description, Location loc) {
        if (writer == null || !isWriterReady) {
            LogUtil.loggerI(GpsService.TAG, "Writer not initialized, cannot write annotation");
            return;
        }
        ...
        writer.flush();
        LogUtil.loggerI(GpsService.TAG, "Written annotation: " + description);
    }
    ...
    public void close() {
        if (writer != null) {
            ...
            writer = null;
            isWriterReady = false;
            LogUtil.loggerI(GpsService.TAG, "Closed log file: " + currentFilePath);
        }
    }
