...
/**
 * 基于 Netty 实现的 TCP Socket 客户端，主要用于向指定服务器发送 GPS 定位数据，并支持断线重连、心跳维持、数据加密和连接状态监听等功能
 *
 * @author fz
 * @version 1.0
 * @since 1.0
 * @created 2023/10/19 13:31
 */
public class GPSSocketServer {
    ...
    private GPSSocketServer() {
        ...
        LogUtil.loggerI(TAG, "----------------GPSSocketServer-----------------");
    }
    ...
    private void connectServer(int retryTimes, @NotNull SocketCallbackListener socketCallbackListener) {
        LogUtil.loggerI(TAG, "开始尝试连接，第" + (errorCount + 1) + "次");
        //避免重连时大量重新创建对象
        if (bootstrap == null) {
        ...
        try {
            bootstrap.connect(TCP_IP, TCP_PORT).sync()
                    .addListener((ChannelFutureListener) future -> {
                        LogUtil.loggerI(TAG, "------------------addListener：" + future.isSuccess() + "------------------");
                        if (future.isSuccess()) {
                            ...
                            LogUtil.loggerI(TAG, "连接失败");
                            errorCount++;
                            if (errorCount >= retryTimes) {
                                connectionRunnable = null;
                                socketCallbackListener.callBack(false, "连接失败已达最大次数，将不会再重试！");
                                LogUtil.loggerI(TAG, "连接失败已达最大次数，将不会再重试！");
                                return;
                            }
                            LogUtil.loggerI(TAG, "第" + (errorCount - 1) + "次连接失败," + (RETRY_INTERVAL / 1000) + "秒后将重试第" + (errorCount + 1) + "次");
            ...
            LogUtil.loggerI(TAG, "第" + (errorCount) + "次连接异常," + e);
            LogUtil.loggerI(TAG, (RETRY_INTERVAL / 1000) + "秒后将重试第" + (errorCount + 1) + "次");
            handler.postDelayed(connectionRunnable = new ConnectionRunnable(retryTimes, socketCallbackListener), RETRY_INTERVAL);
        }
    }
    ...
    public void sendMsg(String msg, @NotNull SocketCallbackListener socketCallbackListener) {
        SendMessageThreadExecutor.getInstance().execute(() -> {
            ...
            LogUtil.loggerI(TAG, "sendMsg发送消息内容：" + msg);
            //转换字节发送，
            ...
            socketChannel.writeAndFlush(buffer)
                    .addListener((ChannelFutureListener) future -> {
                        LogUtil.loggerI(TAG, "------------------sendMsg是否成功：" + future.isSuccess() + "------------------");
                        if (future.isSuccess()) {
